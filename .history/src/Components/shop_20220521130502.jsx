...
export class shop extends Component {
    render() {
    const {mobiles,onCart,cart,onRemove}=this.props;
    return (
      <React.Fragment>
        ...
        <div className='row text-center'>
          {mobiles.map((p1,index) => {
            ...
            return(
              ...
              <div className="col-4 border text-center bg-success m-2">
                       <h4>{name}({desc[0]})</h4>
                       <h5>{desc[1]}</h5>
          ...
          <div className="container">{cart.map((ele,index)=><ul><li>{ele.name},Quantity:{ele.qty}<button className="btn btn-primary btn-sm m-2"onClick={() =>onRemove(index)}>Remove from Cart</button></li></ul>)
          }</div>}
