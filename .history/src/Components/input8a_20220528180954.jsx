...
export class input8a extends Component {
    ...
    handelSubmitMarks=(marks,index)=>{
      let s1={...this.state};
    //   s1.total.push(marks);
      let str=s1.marks.reduce((acc,curr)=>acc+parseInt(curr.math)+parseInt(curr.eng)+parseInt(curr.comp)+parseInt(curr.science),0)
  ...
  render() {
      const{students,view,viewIndex,totIndex,name,totalMarks}=this.state;
    ...
    return(
        ...
        :
          <div className="container">
             ...
             {students.map((ele,index)=>
                ...
                <td className="col1">{totalMarks}</td>
     ...
     :view===3?<Input8c marks={marks} onSubmitMarks={this.handelSubmitMarks}totIndex={totIndex}name={name}/>:""
     }
