...
export class input8a extends Component {
    ...
    handelSubmitMarks=(marks)=>{
      ...
      s1.total.push(marks);
      this.setState(s1);
      let str=s1.total.reduce((acc,curr)=>acc+curr.math+curr.eng+curr.comp+curr.science,0)
      console.log(str)
  ...
  render() {
      const{students,view,viewIndex,totIndex}=this.state;
      let student={name:"",course:"",year:""};
      let marks={math:0,eng:0,comp:0,science:0}
    return(
        <React.Fragment>
     ...
     :view===3?<Input8c marks={marks} onSubmitMarks={this.handelSubmitMarks}index={totIndex}/>:""
     }
