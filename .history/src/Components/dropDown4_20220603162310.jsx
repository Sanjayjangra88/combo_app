...
export class dropDown4 extends Component {
    ...
    showLaptops=()=>{
        ...
        const laptop1 = 
        brand.length > 0
         ? laptops.filter((ele) => ele.brand===brand )
         : laptops;

        const laptop2 =
         ram.length>0 
         ? laptop1.filter((ele)=>ele.ram===ram)
         : laptop1;
  ...
  render() {
      ...
      const{brand,ram}=optionSell;
    //   console.log(brand);
    return (
      <div className='container'>
          <div className="row">
              <div className="col-3 border bg-light">
                  <LeftPanel optionArry={optionArry}optionSell={optionSell}onChangeOptions={this.handelChangeOptions}remove={this.remove}brand={brand}ram={ram}/>
              </div>
