...
export class Evnt8 extends Component {
    ...
    tot2 =(p1,num)=>{
      let s1 ={...this.state}
      console.log(p1);
        s1.operator=s1.operator=num;
        s1.expression=s1.expression=num(p1);
  ...
  render() {
    ...
    return (
          ...
          <React.Fragment>
            ...
            <button className="btn btn-primary m-2"id="abc"onClick={() => this.tot2("+")}>Add</button>
            <button className="btn btn-primary m-2"id="bcd"onClick={() => this.tot2("*")}>Multiply</button>
            </div>
            {numbers.map((p1) => (
