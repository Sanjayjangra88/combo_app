    ...
    return (
      <div className='container'>
        ...
        {experiences.map((ele)=><option>{ele}</option>)}
          ...
          <div className="form-group">
          <label>Names</label>
          <input type="radio" className="form-control"name="name"id="name"value={name}onChange={this.handelChange}>
        <option disabled value="">Select the Name</option>
        {managers.map((ele)=><option>{ele}</option>)}
          </input>
          </div>
      </div>
    )
