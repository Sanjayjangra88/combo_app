...
export class dropdown5 extends Component {
    ...
    handelChange=(e)=>{
        ...
        let s1={...this.state};
        // input.type==="radio"
        // ? input.name==="name" ? (s1.manager.name = this.updateCBs(input.checked,input.value,s1.managers.name))
        //     :(s1.managers[input.name]=input.checked)
        //     :(s1.managers[input.name]=input.value);
  ...
  render() {
    ...
    return (
      <div className='container'>
        ...
        <label className="form-check-label font-weight-bold">Choose Name</label><br/>
                {managers.map((ele)=>(
