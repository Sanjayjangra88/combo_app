...
export class input8a extends Component {
    ...
    handelSubmitMarks=(marks,name)=>{
      let s1={...this.state};
    s1.students[s1.totIndex].totalMarks.push(marks)
      this.setState(s1)
      console.log(s1.students[s1.totIndex].totalMarks)
     let arr1= s1.students[s1.totIndex].totalMarks.reduce((acc,curr)=>acc+parseInt(curr.math)+parseInt(curr.eng)+parseInt(curr.comp)+parseInt(curr.science),0)
console.log(arr1)
  ...
  render() {
      const{students,view,viewIndex,totIndex,name,totalMarks,index}=this.state;
    ...
    return(
     ...
     :view===3?<Input8c marks={marks} onSubmitMarks={this.handelSubmitMarks}totIndex={totIndex}name={name}index={index}/>:""
     }
