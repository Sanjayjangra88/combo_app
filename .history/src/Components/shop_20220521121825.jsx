      ...
      <React.Fragment>
       ...
       <div className="row">
        ...
        <div className='row text-center'>
          ...
          <div className="container">{cart.map((ele)=><ul><li>{ele.name},Quantity:{ele.qty}</li></ul>)
          }</div>}
          {(cart.length<1)?'':<div className='container'>Number of Items in Cart : {cart.reduce((acc,curr)=>acc+curr.qty,0)}</div>}
          {(cart.length<1)?'':<div className='container'>Value of Cart : {cart.reduce((acc,curr)=>acc+curr.price,0)}</div>}
