...
export class Evnt10 extends Component {
    ...
    sort=(colnum)=>{
      const {products,p1}=this.state;
      let a2 ={...this.state}
 
     if(colnum===0){
     a2= products && p1.sort((p1,p2)=>p1.name.localeCompare(p2.name));
     a2.cng="Sort by name";
    //  a2.cng2="Sort by name";
     this.setState(a2.products);
    }
    else if(colnum===1){
  ...
  render() {
      const {products,cng,p1,cng2}=this.state;
      return (
        <React.Fragment>
          <h3>{cng}</h3><br/>
