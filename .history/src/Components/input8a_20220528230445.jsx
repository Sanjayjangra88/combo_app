...
export class input8a extends Component {
    ...
    handelSubmitMarks=(marks)=>{
        let s1={...this.state};
        s1.total.push(marks)
        s1.total.reduce((acc,curr)=>acc+parseInt(curr.math)+parseInt(curr.eng)+parseInt(curr.comp)+parseInt(curr.science),0)
        s1.students[s1.totIndex].totalMarks = this.total;
        s1.view=0;
        this.setState(s1);
