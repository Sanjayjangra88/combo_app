...
export class Evnt12 extends Component {
    ...
    delete=(title)=>{
        ...
        let a2 ={...this.state}
        console.log(title);
        let str= movies.findIndex((ele)=>ele.title===title)
        movies.splice(str,1)
        // // a2.movies.length==0 ?"hy":;
        // console.log(a2.movies.length)
        this.setState(a2);
  ...
  render() {
      ...
      return (
        ...
        <React.Fragment>
        {a2.movies.length==0?a2.movies="There is no movies":<h4 className='text-center'>Showing {movies.length} Movies</h4>}
